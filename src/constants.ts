...
export const AVENGE_MIN_WEIGHT = 1.5; // only a deed this heavy (a sack or worse) settles an old ledger
export const AVENGE_MIN_YEARS = 10; // a wound must fester before answering it is vengeance, not just the same war
export const AVENGED_FACTOR = 0.25; // an avenged wound keeps this fraction of its weight — answered, not erased
...
// --- Angels and demons: intelligent powers that hold office ---
export const DEMON_CHANCE = 0.003; // per year, when none is abroad; x4 where fires burn to darker powers, x2 with a dark prophet crying
export const DEMON_REACH = 6; // cells from which a demon can see a throne
export const DEMON_USURP_CHANCE = 0.5; // per year, a demon beside a weak throne takes it
export const DEMON_MUSTER = 1.3; // hosts of the damned: the levy a demon king raises
export const DEMON_TIRE_YEARS = 60; // a demon king may tire of a throne after this...
export const DEMON_TIRE_CHANCE = 0.05; // ...at this chance per year
export const CHALLENGE_CHANCE = 0.15; // per year, a champion in reach rides against a demon king
export const CHALLENGE_REACH = 14; // cells from which a devout neighbor's champion will ride
export const ANGEL_CAP = 3; // guardians in the world at once
...
// --- Economy v1: goods, routes, treasuries, and what gold buys ---
export const PRICE: Record<string, number> = { grain: 3, timber: 2, ore: 4, gold: 6, furs: 3, fish: 2 }; // wealth a unit of trade earns the seller
export const ROUTES_PER_NATION = 4; // wagon routes a nation can keep open at once
export const ROUTE_OPEN_CHANCE = 0.5; // per year, a matched surplus and deficit becomes a route
export const ROUTE_IDLE_CLOSE = 6; // years without goods moving before a route closes
export const ROUTE_KEEP_MIN = 1.05; // an open route keeps flowing while the seller has this much of its need: routes are sticky
export const BOOM_NOTE_YEARS = 25; // a people's fortunes are chronicled at most this often
export const ROUTE_SELL_MIN = 1.25; // a seller must have this much of its need before it exports
export const ROUTE_BUY_MAX = 0.8; // a buyer must be short by this much before it imports
export const TREASURY_RATE = 0.1; // share of the year's wealth a nation lays by
export const TREASURY_CAP = 120;
export const GRANARY_COST = 12; // treasury spent on a granary
export const GRANARY_MAX = 3;
export const GRANARY_RELIEF = 0.15; // starvation mortality reduced per granary
export const MARKET_COST = 20; // treasury spent to make the seat a market town (needs two routes)
export const MARKET_WEALTH = 3; // wealth a market brings each year
export const MERC_COST = 25; // treasury spent to hire sellswords
export const MERC_FRACTION = 0.1; // share of the hired settlement that marches for gold
export const MERC_CHANCE = 0.3; // per year, a nation at war with the gold hires
export const CARAVAN_SPEED = 3; // cells per season
export const CARAVAN_RAID_CHANCE = 0.25; // per season, a beast within reach of a caravan takes it
export const CARAVAN_LOG_YEARS = 10; // per route, raided caravans chronicle at most this often
export const PROSPERITY_BOOM = 1.2; // prosperity at which a people is chronicled as growing rich
export const PROSPERITY_BUST = 0.6; // and at which the markets stand empty
export const PROSPERITY_FOOD = 0.05; // foodSat lift per point of prosperity above 1 (capped)
export const PROSPERITY_FOOD_CAP = 0.04;
export const ARMS_MIN = 0.85; // battle strength of a people with no iron at all...
export const ARMS_SPAN = 0.3; // ...rising this much when their need for ore is met
export const TIMBER_ROADS_MIN = 0.5; // below this share of their timber need, a people lays no roads
export const GREED_WAR_RATIO = 2; // a neighbor this many times richer is worth a war to the warlike
export const GREED_WAR_CHANCE = 0.12; // per year, once a warlike leader covets it
export const TRIBUTE_PER_SETTLEMENT = 1; // wealth a master draws from each conquered settlement per year
export const ENRICH_RADIUS = 1; // cells of new vein
